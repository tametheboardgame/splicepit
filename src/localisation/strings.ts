const EN_GB = {
  ...
  'splice.instructions': 'Each disposable subject gets one experiment, then it is retired and replaced with a fresh clean animal. A valued main creature unlocks after at least one test result for that source. Once only one physical dose remains, the bench reserves it for your main so testing cannot accidentally consume the commit dose.',
  ...
  'splice.testSubjectSpent': 'DISPOSABLE SUBJECT SPENT\nA test animal is used for one experiment only. Its result remains in the records, but further testing uses a fresh clean subject.',
  'splice.mainDoseReserved': 'MAIN DOSE RESERVED\nYou have test evidence and only one physical dose remains. Further disposable testing is blocked so you cannot accidentally consume the dose needed for your valued main. Select VALUED MAIN when you are ready.',
  'splice.testReplacementReady': 'DISPOSABLE TEST COMPLETE. This subject has been retired from active stock and animal holding has supplied a fresh clean animal of the same species.',

const EN_GB = {
  ...
  'splice.noKnownWarnings': '- No specific visible warning identified.',
  'splice.unknownRemain': 'UNKNOWN FACTORS REMAIN',
  'splice.unknownReduced': 'UNKNOWN FACTORS REDUCED, NOT ELIMINATED',
  'splice.forecastBody': 'RESEARCH      {confidence}\nOBSERVATIONS  {observations}\n\nVIABLE EXPRESSION\nroughly {viableLow}–{viableHigh}%\n\nADVERSE OUTCOME\nroughly {adverseLow}–{adverseHigh}%\n\nCOMPATIBILITY {compatibility}\n{unknown}\n\nKNOWN WARNINGS\n{warnings}\n\nREAGENT STOCK {reagent}',
  ...
  'splice.outcomeBody': 'LATEST OUTCOME\n{outcome}\n\nESTABLISHED\n{established}\n\nSTABILITY {before}% -> {after}%\nINJURY {injury}\nMUTATION {mutation}\nCREATURE HISTORY {history} attempt(s)',
  'splice.irreversibleWarning': 'MAIN CREATURE COMMIT ARMED.\n\nThis will consume material and permanently append the result to this creature. Rejection, injury, mutation, permanent damage and death are all possible. Confirm only if you accept that outcome.',
  'splice.outcomePrompt': 'Run tests on common animals, compare the records, then decide whether the evidence is good enough to risk the main creature.',

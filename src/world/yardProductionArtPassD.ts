import { drawEnvironmentContactShadow, drawPixelRect } from '../environment/environmentArtLanguage.js';
...
const P = {
  ...
  waterLight: '#8fc0b5',
  waterDark: '#416f72',
  shadow: 'rgba(31,40,35,0.24)',
  light: 'rgba(255,226,158,0.12)',
} as const;
...
const DP = {
  ...
  waterLight: '#557a73',
  waterDark: '#293f46',
  shadow: 'rgba(18,20,22,0.44)',
  light: 'rgba(126,74,85,0.08)',
} as const;

type Palette = typeof P;

function rect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, colour: string): void {
  drawPixelRect(ctx, Math.round(x), Math.round(y), Math.round(w), Math.round(h), colour);
}
...
function polygon(ctx: CanvasRenderingContext2D, points: readonly (readonly [number, number])[], colour: string): void {
  ...
  for (let i = 1; i < points.length; i += 1) {
    const p = points[i];
    if (p) ctx.lineTo(Math.round(p[0]), Math.round(p[1]));
  }
  ctx.closePath();
  ctx.fill();
}
...
function drawGround(ctx: CanvasRenderingContext2D, p: Palette, dark: boolean): void {
  // Opaque plate intentionally supersedes the old Yard core rather than decorating it.
  ...
  // Main player desire line: arrival -> work yard -> east route.
  polygon(ctx, [
    [X + 420, Y + 540], [X + 386, Y + 488], [X + 354, Y + 438], [X + 330, Y + 384],
    [X + 338, Y + 334], [X + 390, Y + 310], [X + 474, Y + 300], [X + 582, Y + 300],
    [X + 674, Y + 322], [X + 782, Y + 328], [X + 960, Y + 318], [X + 960, Y + 382],
    [X + 770, Y + 390], [X + 660, Y + 382], [X + 560, Y + 360], [X + 472, Y + 360],
    [X + 414, Y + 374], [X + 408, Y + 420], [X + 438, Y + 478], [X + 468, Y + 540],
  ], p.dirtDark);
  polygon(ctx, [
    [X + 430, Y + 540], [X + 402, Y + 482], [X + 372, Y + 432], [X + 366, Y + 394],
    [X + 384, Y + 356], [X + 454, Y + 334], [X + 562, Y + 330], [X + 666, Y + 352],
    [X + 778, Y + 356], [X + 960, Y + 346], [X + 960, Y + 370], [X + 776, Y + 378],
    [X + 662, Y + 370], [X + 558, Y + 346], [X + 462, Y + 348], [X + 394, Y + 370],
    [X + 390, Y + 414], [X + 418, Y + 476], [X + 448, Y + 540],
  ], p.dirtLight);

  // Traffic and hose wear is irregular and follows actual work zones.
  for (const [px, py, w] of [
    [X + 376, Y + 401, 54], [X + 426, Y + 434, 72], [X + 490, Y + 466, 46],
    [X + 565, Y + 352, 64], [X + 650, Y + 365, 42], [X + 744, Y + 360, 60],
  ] as const) {
    rect(ctx, px, py, w, 3, p.mud);
    rect(ctx, px + 8, py + 7, Math.max(10, w - 18), 2, p.dirt);
  }
  ...
  if (dark) {
    polygon(ctx, [[X + 486, Y + 416], [X + 550, Y + 404], [X + 614, Y + 422], [X + 594, Y + 448], [X + 522, Y + 444]], '#4c303b');
    rect(ctx, X + 510, Y + 426, 54, 5, '#713845');
    rect(ctx, X + 860, Y + 214, 72, 8, '#53303d');
  }
}
...
function drawWorkshop(ctx: CanvasRenderingContext2D, now: number, p: Palette, dark: boolean): void {
  const bx = X + 54;
  const by = Y + 74;
  drawEnvironmentContactShadow(ctx, bx + 8, by + 164, 414, 13, dark ? 0.46 : 0.28);

  // Irregular patched facade and roof, sized to the existing collider footprint.
  polygon(ctx, [[bx + 0, by + 48], [bx + 34, by + 24], [bx + 118, by + 16], [bx + 176, by + 2], [bx + 260, by + 14], [bx + 332, by + 8], [bx + 430, by + 38], [bx + 418, by + 72], [bx + 12, by + 72]], p.roofDark);
  polygon(ctx, [[bx + 16, by + 44], [bx + 78, by + 28], [bx + 164, by + 18], [bx + 250, by + 28], [bx + 330, by + 22], [bx + 410, by + 44], [bx + 398, by + 58], [bx + 28, by + 58]], p.roof);
  for (const [px, py, w] of [[44, 35, 52], [136, 25, 66], [248, 31, 54], [332, 33, 44]] as const) rect(ctx, bx + px, by + py, w, 4, p.roofLight);
  ...
  // Window banks feel like working lab windows, not generic blue squares.
  ...
  // Service door and battered warning board.
  ...
  // Greenhouse/handling annex with overhead service pipe.
  rect(ctx, bx + 312, by + 72, 96, 92, p.steelDark);
  rect(ctx, bx + 318, by + 78, 84, 80, p.glass);
  rect(ctx, bx + 324, by + 84, 28, 26, p.glassLight);
  for (let gx = 330; gx <= 392; gx += 20) rect(ctx, bx + gx - 312, by + 78, 3, 80, p.steel);
  rect(ctx, bx + 328, by + 132, 58, 14, p.timber);
  rect(ctx, bx + 338, by + 118, 7, 18, dark ? '#6b4552' : '#6f9f5d');
  rect(ctx, bx + 330, by + 112, 22, 10, dark ? '#875662' : '#9cc970');
  rect(ctx, bx + 365, by + 122, 6, 14, dark ? '#6b4552' : '#6f9f5d');
  rect(ctx, bx + 358, by + 116, 20, 10, dark ? '#8f5966' : '#bb7b8a');
  ...
  // Chimney and stitched roof repairs add silhouette character.
  ...
  if (dark) {
    line(ctx, bx + 345, by + 18, bx + 351, by + 62, '#6f3949', 5);
    line(ctx, bx + 351, by + 62, bx + 330, by + 89, '#7c4050', 4);
    line(ctx, bx + 351, by + 62, bx + 378, by + 90, '#643742', 3);
    polygon(ctx, [[bx + 112, by + 146], [bx + 170, by + 134], [bx + 210, by + 152], [bx + 178, by + 160], [bx + 122, by + 158]], '#57323d');
  }
}

function drawAnimalHandling(ctx: CanvasRenderingContext2D, now: number, p: Palette, dark: boolean): void {
  const x = X + 506;
  const y = Y + 54;
  drawEnvironmentContactShadow(ctx, x + 4, y + 128, 194, 10, dark ? 0.44 : 0.24);

  // Pen frame, gates, feed rail and one deliberately odd specimen silhouette.
  ...
  rect(ctx, x + 30, y + 96, 44, 8, dark ? '#5e4854' : '#7ca278');

  if (!dark) {
    // Small rabbit-goat test animal: recognisable but not a hero asset.
    ...
    polygon(ctx, [[x + 88, y + 78], [x + 132, y + 64], [x + 154, y + 80], [x + 142, y + 104], [x + 104, y + 100]], '#6d4352');
    line(ctx, x + 112, y + 68, x + 98, y + 42, '#8a5260', 5);
    line(ctx, x + 132, y + 68, x + 150, y + 38, '#8a5260', 5);
    rect(ctx, x + 142, y + 82, 4, 4, '#b8787a');
...
function drawVatAndCage(ctx: CanvasRenderingContext2D, now: number, p: Palette, dark: boolean): void {
  // South-west specimen vat, aligned to existing collider.
  const vx = X + 68;
  const vy = Y + 298;
  drawEnvironmentContactShadow(ctx, vx + 4, vy + 132, 158, 9, dark ? 0.46 : 0.28);
  rect(ctx, vx, vy + 4, 164, 142, p.steelDark);
  rect(ctx, vx + 8, vy + 12, 148, 126, p.steel);
  rect(ctx, vx + 18, vy + 18, 128, 106, p.glass);
  rect(ctx, vx + 26, vy + 28, 112, 88, dark ? '#684b60' : '#84bd84');
  ...
  if (!dark) {
    ...
    polygon(ctx, [[vx + 52, vy + 64], [vx + 90, vy + 48], [vx + 118, vy + 68], [vx + 106, vy + 96], [vx + 68, vy + 90]], '#83505e');
    line(ctx, vx + 82, vy + 58, vx + 70, vy + 34, '#9b6170', 4);
    line(ctx, vx + 96, vy + 58, vx + 112, vy + 30, '#9b6170', 4);
    rect(ctx, vx + 122, vy + 104, 14, 8, '#733743');
  }

  // Low quarantine cage, south-centre.
  const cx = X + 234;
  const cy = Y + 388;
  drawEnvironmentContactShadow(ctx, cx + 4, cy + 70, 108, 8, dark ? 0.42 : 0.22);
  ...
  if (!dark) {
    ...
    polygon(ctx, [[cx + 34, cy + 48], [cx + 66, cy + 30], [cx + 86, cy + 42], [cx + 80, cy + 62], [cx + 48, cy + 62]], '#764858');
    line(ctx, cx + 64, cy + 34, cx + 76, cy + 18, '#8b5664', 3);
  }
}
...
function drawServiceBench(ctx: CanvasRenderingContext2D, now: number, p: Palette, dark: boolean): void {
  const x = X + 452;
  const y = Y + 228;
  drawEnvironmentContactShadow(ctx, x + 2, y + 64, 156, 9, dark ? 0.46 : 0.26);
  ...
  rect(ctx, x + 41, y - 5, 12, 7, dark ? '#764858' : '#8ec678');
  ...
  if (dark) {
    line(ctx, x + 165, y + 12, x + 178, y + 28, '#75404f', 4);
    line(ctx, x + 178, y + 28, x + 164, y + 52, '#8c4c5c', 4);
...
function drawPond(ctx: CanvasRenderingContext2D, now: number, p: Palette, dark: boolean): void {
  ...
  for (const [wx, wy, w] of [[54, 46, 34], [112, 58, 42], [66, 98, 46], [126, 106, 24]] as const) rect(ctx, x + wx + shift, y + wy, w, 3, p.waterLight);
  if (dark) {
    polygon(ctx, [[x + 78, y + 76], [x + 112, y + 66], [x + 142, y + 78], [x + 130, y + 98], [x + 92, y + 98]], '#613846');
    line(ctx, x + 110, y + 80, x + 126, y + 58, '#7c4655', 4);
  }
}

function drawLightingAndSmallStory(ctx: CanvasRenderingContext2D, p: Palette, dark: boolean): void {
  ctx.save();
  ctx.globalAlpha = dark ? 0.32 : 0.38;
  polygon(ctx, [[X + 314, Y + 178], [X + 656, Y + 188], [X + 742, Y + 372], [X + 402, Y + 410]], p.light);
  ctx.restore();

  // Waste bins, hose coil and failed repairs are clustered, not sprinkled randomly.
  ...
  ctx.save();
  ctx.strokeStyle = dark ? '#684151' : '#3e625c';
  ctx.lineWidth = 4;
  ctx.beginPath();
  ...
  if (!dark) {
    ...
    polygon(ctx, [[X + 594, Y + 450], [X + 640, Y + 440], [X + 676, Y + 456], [X + 650, Y + 470], [X + 604, Y + 466]], '#4f303b');
    line(ctx, X + 636, Y + 448, X + 648, Y + 420, '#743d4c', 4);
  }
}
...
function drawForeground(ctx: CanvasRenderingContext2D, playerFeetY: number, p: Palette, dark: boolean): void {
  ...
  if (dark) {
    line(ctx, X + 166, Y + 480, X + 186, Y + 500, '#6d3b49', 4);
    line(ctx, X + 186, Y + 500, X + 202, Y + 490, '#834a58', 3);
    polygon(ctx, [[X + 312, Y + 476], [X + 350, Y + 470], [X + 370, Y + 486], [X + 334, Y + 493]], '#56313e');
  }
}

export function drawPassDYardBrightForeground(ctx: CanvasRenderingContext2D, playerFeetY: number): void {
  drawForeground(ctx, playerFeetY, P, false);
}

export function drawPassDYardDarkForeground(ctx: CanvasRenderingContext2D, playerFeetY: number): void {
  drawForeground(ctx, playerFeetY, DP, true);
}

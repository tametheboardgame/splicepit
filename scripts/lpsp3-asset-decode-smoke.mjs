...
try {
  ...
  const result = await evaluate(`(async () => {
    const response = await fetch('/generated/lpsp3/local-pit-bright-base.jpg', { cache: 'no-store' });
    if (!response.ok) throw new Error('LPSP-3 base fetch failed: ' + response.status);
  ...
  if (!result?.complete || result.bytes !== expectedBytes || result.width !== expectedWidth || result.height !== expectedHeight) {
    throw new Error(`LPSP-3 browser decode mismatch: ${JSON.stringify(result)}`);
  }

  console.log(`LPSP-3 browser decode passed: ${result.width}x${result.height}, ${result.bytes} bytes`);
  ws.close();
  cleanup();
} catch (error) {
  cleanup();

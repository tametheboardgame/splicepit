...
const BASE_CHUNKS = Array.from({ length: 7 }, (_, index) =>
  `yard-bright-base.part${String(index).padStart(2, '0')}.txt`,
);
...
async function main() {
  ...
  if (WRITE_OUTPUTS) {
    ...
    console.log(`YSP-3 Yard assets materialised: ${WIDTH}x${HEIGHT}, base sha256 ${manifest.base.sha256}`);
  } else {
    console.log(`YSP-3 Yard source validated: ${WIDTH}x${HEIGHT}, base sha256 ${manifest.base.sha256}`);
  }
}

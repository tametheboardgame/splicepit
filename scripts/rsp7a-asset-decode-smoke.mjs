...
const chromePort = 9267;
const gamePort = 8117;
const expected = {
  bright: { path: '/generated/rsp7a/route-bright-base.webp', bytes: 392486 },
  dark: { path: '/generated/rsp7a/route-dark-base.webp', bytes: 334852 },
};
const expectedWidth = 1024;
const expectedHeight = 683;
...
try {
  ...
  for (const [state, asset] of Object.entries(expected)) {
    const result = await evaluate(`(async () => {
      ...
      if (!response.ok) throw new Error('RSP-7A ${state} fetch failed: ' + response.status);
    ...
    if (!result?.complete || result.bytes !== asset.bytes || result.width !== expectedWidth || result.height !== expectedHeight) {
      throw new Error(`RSP-7A ${state} browser decode mismatch: ${JSON.stringify(result)}`);
    }
  }

  console.log('RSP-7A browser decode passed: Bright and Dark 1024x683 approved masters.');
  ws.close();
  cleanup();
} catch (error) {
  cleanup();
